...
auto draw_menu_background(Core::State::AppState& state, const D2D1_RECT_F& rect) -> void {
    ...
    D2D1_ROUNDED_RECT rounded_rect = D2D1::RoundedRect(rect, static_cast<float>(layout.border_radius), static_cast<float>(layout.border_radius));
...
auto draw_menu_items(Core::State::AppState& state, const D2D1_RECT_F& rect) -> void {
    ...
    for (size_t i = 0; i < menu_state.items.size(); ++i) {
        ...
        if (item.type == Types::MenuItemType::Separator) {
            ...
            D2D1_RECT_F separator_rect = D2D1::RectF(rect.left + static_cast<float>(layout.text_padding), current_y, rect.right - static_cast<float>(layout.text_padding), current_y + separator_height);
            draw_separator(state, separator_rect);
            current_y += separator_height;
        } else {
            float item_height = static_cast<float>(layout.item_height);
            D2D1_RECT_F item_rect = D2D1::RectF(rect.left, current_y, rect.right, current_y + item_height);
...
auto draw_single_menu_item(Core::State::AppState& state, const Types::MenuItem& item, const D2D1_RECT_F& item_rect, bool is_hovered) -> void {
    const auto& menu_state = *state.context_menu;
    const auto& d2d = state.app_window->d2d_context;
    const auto& layout = menu_state.layout;
    ...
    D2D1_RECT_F text_rect = D2D1::RectF(item_rect.left + static_cast<float>(layout.text_padding), item_rect.top, item_rect.right - static_cast<float>(layout.text_padding), item_rect.bottom);
    ID2D1SolidColorBrush* text_brush = item.is_enabled ? menu_state.text_brush : menu_state.separator_brush;
    menu_state.render_target->DrawText(item.text.c_str(), static_cast<UINT32>(item.text.length()), d2d.text_format, text_rect, text_brush);
    ...
    if (item.has_submenu()) {
        ...
        D2D1_RECT_F arrow_rect = D2D1::RectF(arrow_x, arrow_y, arrow_x + arrow_size, arrow_y + arrow_size);
        menu_state.render_target->DrawText(L"▶", 1, d2d.text_format, arrow_rect, text_brush);
        ...
        D2D1_RECT_F check_rect = D2D1::RectF(check_x, check_y, check_x + check_size, check_y + check_size);
...
auto draw_submenu_background(Core::State::AppState& state, const D2D1_RECT_F& rect) -> void {
    ...
    D2D1_ROUNDED_RECT rounded_rect = D2D1::RoundedRect(rect, static_cast<float>(layout.border_radius), static_cast<float>(layout.border_radius));
    menu_state.submenu_render_target->FillRoundedRectangle(rounded_rect, menu_state.submenu_white_brush);
}
...
auto draw_submenu_single_item(Core::State::AppState& state, const Types::MenuItem& item, const D2D1_RECT_F& item_rect, bool is_hovered) -> void {
    const auto& menu_state = *state.context_menu;
    const auto& d2d = state.app_window->d2d_context;
    const auto& layout = menu_state.layout;
    if (is_hovered && item.is_enabled) {
    ...
    D2D1_RECT_F text_rect = D2D1::RectF(item_rect.left + static_cast<float>(layout.text_padding), item_rect.top, item_rect.right - static_cast<float>(layout.text_padding), item_rect.bottom);
    ID2D1SolidColorBrush* text_brush = item.is_enabled ? menu_state.submenu_text_brush : menu_state.submenu_separator_brush;
    menu_state.submenu_render_target->DrawText(item.text.c_str(), static_cast<UINT32>(item.text.length()), d2d.text_format, text_rect, text_brush);
    if (item.is_checked) {
        float check_size = static_cast<float>(layout.font_size) * 0.8f;
        float check_x = item_rect.right - static_cast<float>(layout.text_padding) - check_size;
        float check_y = item_rect.top + (item_rect.bottom - item_rect.top - check_size) / 2;
...
auto draw_submenu_separator(Core::State::AppState& state, const D2D1_RECT_F& separator_rect) -> void {
    const auto& menu_state = *state.context_menu;
    menu_state.submenu_render_target->FillRectangle(separator_rect, menu_state.submenu_separator_brush);
}
