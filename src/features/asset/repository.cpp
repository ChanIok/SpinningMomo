...
auto list_asset(Core::State::AppState& app_state, const Types::ListParams& params)
  ...
  auto items_result =
      Core::Database::query<Types::Asset>(*app_state.database, sql, final_params);
  if (!items_result) {
    return std::unexpected("Failed to query asset items: " + items_result.error());
  }

  // 构建响应
  Types::AssetListResponse response;
...
auto get_asset_stats(Core::State::AppState& app_state, const Types::GetStatsParams& params)
  ...
  auto type_result =
      Core::Database::query<Types::AssetTypeCountResult>(*app_state.database, type_sql, where_params);
  if (type_result) {
...
auto filepath_exists(Core::State::AppState& app_state, const std::string& filepath)
  ...
  std::string sql = "SELECT COUNT(*) FROM assets WHERE filepath = ? AND deleted_at IS NULL";
  std::vector<Core::Database::Types::DbParam> params = {filepath};
...
auto batch_create_asset(Core::State::AppState& app_state,
  ...
  for (const auto& item : items) {
    auto result = create_asset(app_state, item);
    if (result) {
...
auto batch_update_asset(Core::State::AppState& app_state,
    ...
    -> std::expected<void, std::string> {
  for (const auto& item : items) {
    auto result = update_asset(app_state, item);
    if (!result) {
