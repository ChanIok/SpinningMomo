      ...
      for (const auto& entry : std::filesystem::recursive_directory_iterator(directory)) {
        try {
          if (entry.is_regular_file()) {
            if (is_supported_media_file(entry.path(), supported_extensions)) {
...
auto detect_media_type(const std::filesystem::path& file_path) -> std::string {
  if (!file_path.has_extension()) {
    return "unknown";
  }
...
auto extract_media_info(Utils::Image::WICFactory& wic_factory,
  ...
  try {
    ...
    // 检测媒体类型
    auto media_type = detect_media_type(file_path);

    Types::MediaInfo info;
    info.file_size = static_cast<int64_t>(file_size);
    info.detected_type = media_type;
...
auto process_single_media_file(Core::State::AppState& app_state,
  ...
  try {
    // 提取媒体信息
    auto media_info_result = extract_media_info(wic_factory, file_path);
    if (!media_info_result) {
      return std::unexpected("Failed to extract media info: " + media_info_result.error());
    }
    auto media_info = media_info_result.value();

    // 创建 MediaItem
    Types::MediaItem item;
    item.filename = file_path.filename().string();
    item.filepath = file_path.string();
    item.relative_path = calculate_relative_path(file_path, base_directory);
    item.type = media_info.detected_type;
    item.width = media_info.width;
    item.height = media_info.height;
    item.file_size = media_info.file_size;
    item.mime_type = media_info.mime_type;
...
auto process_media_files(Core::State::AppState& app_state, Utils::Image::WICFactory& wic_factory,
  ...
  try {
    std::vector<Types::MediaItem> items;
    items.reserve(file_paths.size());

    for (const auto& file_path : file_paths) {
      auto item_result = process_single_media_file(app_state, wic_factory, file_path,
...
auto is_file_already_indexed(Core::State::AppState& app_state,
  ...
  return Database::filepath_exists(app_state, file_path.string());
}
...
auto needs_update(Core::State::AppState& app_state, const std::filesystem::path& file_path)
  ...
  try {
    // 获取现有记录
    auto existing_item_result = Database::get_media_item_by_filepath(app_state, file_path.string());
...
auto scan_directory(Core::State::AppState& app_state, const Types::ScanOptions& options)
  ...
  try {
    ...
    Logger().info("Starting media scan with {} directories", options.directories.size());
    ...
    std::vector<Types::MediaItem> all_new_items;
    std::vector<Types::MediaItem> all_updated_items;
    ...
    for (const auto& dir_str : options.directories) {
      ...
      // 查找媒体文件
      auto files_result =
          find_media_files(directory, options.supported_extensions, options.recursive);
      if (!files_result) {
        ScanError error;
      ...
      Logger().info("Found {} media files in {}", found_files.size(), directory.string());
      ...
      for (const auto& file_path : found_files) {
        try {
          ...
          // 处理文件
          auto item_result = process_single_media_file(app_state, wic_factory, file_path, directory,
    ...
    if (!all_new_items.empty()) {
      auto batch_create_result = Database::batch_create_media_items(app_state, all_new_items);
      if (!batch_create_result) {
        Logger().error("Failed to batch create media items: {}", batch_create_result.error());
        database_success = false;
        // 不立即返回，先尝试更新操作
      } else {
        Logger().info("Successfully created {} new media items", all_new_items.size());
      }
    }

    if (!all_updated_items.empty()) {
      auto batch_update_result = Database::batch_update_media_items(app_state, all_updated_items);
      if (!batch_update_result) {
        Logger().error("Failed to batch update media items: {}", batch_update_result.error());
        database_success = false;
      } else {
        Logger().info("Successfully updated {} media items", all_updated_items.size());
      }
    }
    ...
    Logger().info("Media scan completed. Processed: {}, New: {}, Updated: {}, Errors: {}",
                  stats.files_processed, stats.new_files_added, stats.files_updated,
                  stats.errors_count);

    return statistics_to_result(stats);
...
auto ScanStatistics::get_duration_string() const -> std::string {
