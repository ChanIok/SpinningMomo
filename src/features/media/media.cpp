...
import Features.Media.Scanner;
import Features.Media.Thumbnail;
import Utils.Image;
import Utils.Logger;
...
auto initialize(Core::State::AppState& app_state) -> std::expected<void, std::string> {
  try {
    ...
    // 确保缩略图目录存在
    auto thumbnail_dir_result = Thumbnail::ensure_thumbnails_directory_exists();
    if (!thumbnail_dir_result) {
      Logger().error("Failed to ensure thumbnails directory: {}", thumbnail_dir_result.error());
      return std::unexpected("Failed to create thumbnails directory: " +
                             thumbnail_dir_result.error());
    }

    Logger().info("Media module initialized successfully");
    return {};

  } catch (const std::exception& e) {
    return std::unexpected("Exception during media module initialization: " +
                           std::string(e.what()));
  }
}
...
auto cleanup(Core::State::AppState& app_state) -> void {
  try {
    Logger().info("Cleaning up media module resources...");
    // 目前没有需要清理的资源
    Logger().info("Media module cleanup completed");
  } catch (const std::exception& e) {
    Logger().error("Exception during media module cleanup: {}", e.what());
  }
}

// ============= 媒体项管理 =============

auto delete_media_item(Core::State::AppState& app_state, const Types::DeleteMediaParams& params)
    -> std::expected<Types::MediaOperationResult, std::string> {
  try {
    // 获取要删除的媒体项
    auto media_item_result = Database::get_media_item_by_id(app_state, params.id);
    if (!media_item_result) {
      return std::unexpected("Failed to get media item: " + media_item_result.error());
    }

    if (!media_item_result->has_value()) {
      Types::MediaOperationResult result;
    ...
    // 删除缩略图
    auto delete_thumbnail_result = Thumbnail::delete_thumbnail(media_item);
    if (!delete_thumbnail_result) {
      Logger().warn("Failed to delete thumbnail for media item {}: {}", params.id,
                    delete_thumbnail_result.error());
    }

    // 删除物理文件（如果请求）
    if (params.delete_file.value_or(false)) {
      std::filesystem::path file_path(media_item.filepath);
      if (std::filesystem::exists(file_path)) {
        std::error_code ec;
        std::filesystem::remove(file_path, ec);
        if (ec) {
          Logger().warn("Failed to delete physical file {}: {}", media_item.filepath, ec.message());
        } else {
          Logger().info("Deleted physical file: {}", media_item.filepath);
        }
      }
    }
    ...
    if (params.delete_file.value_or(false)) {
      delete_result = Database::hard_delete_media_item(app_state, params.id);
    } else {
      delete_result = Database::soft_delete_media_item(app_state, params.id);
    }
    ...
    if (delete_result) {
      ...
      result.message = params.delete_file.value_or(false)
                           ? "Media item and file deleted successfully"
                           : "Media item deleted successfully";
      result.affected_count = 1;
    } else {
      result.success = false;
      result.message = "Failed to delete media item: " + delete_result.error();
      result.affected_count = 0;
    }
    ...
    return std::unexpected("Exception in delete_media_item: " + std::string(e.what()));
  }
}

// ============= 扫描和索引 =============

auto scan_media_directories(Core::State::AppState& app_state, const Types::ScanMediaParams& params)
    -> std::expected<Types::ScanResult, std::string> {
  try {
    // 验证参数
    auto validation_result = validate_scan_params(params);
    ...
    Logger().info("Starting media scan of {} directories", scan_options.directories.size());

    // 执行扫描
    auto scan_result = Scanner::scan_directory(app_state, scan_options);
    if (!scan_result) {
      Logger().error("Media scan failed: {}", scan_result.error());
      return std::unexpected("Scan failed: " + scan_result.error());
    }

    auto result = scan_result.value();
    Logger().info("Media scan completed. New: {}, Updated: {}, Errors: {}", result.new_items,
                  result.updated_items, result.errors.size());

    return result;

  } catch (const std::exception& e) {
    return std::unexpected("Exception in scan_media_directories: " + std::string(e.what()));
  }
}

auto rescan_media_directory(Core::State::AppState& app_state, const std::string& directory_path,
                            bool generate_thumbnails)
    -> std::expected<Types::ScanResult, std::string> {
  try {
    Types::ScanMediaParams params;
    params.directories = {directory_path};
    params.recursive = true;
    params.generate_thumbnails = generate_thumbnails;

    return scan_media_directories(app_state, params);

  } catch (const std::exception& e) {
    return std::unexpected("Exception in rescan_media_directory: " + std::string(e.what()));
  }
}

// ============= 缩略图管理 =============

auto get_thumbnail_data(Core::State::AppState& app_state, const Types::GetThumbnailParams& params)
    -> std::expected<std::vector<uint8_t>, std::string> {
  return Thumbnail::get_thumbnail_data(app_state, params.media_id);
}

auto generate_missing_thumbnails(Core::State::AppState& app_state, uint32_t max_width,
                                 uint32_t max_height)
    -> std::expected<Types::ScanResult, std::string> {
  try {
    ...
    // 获取所有照片类型的媒体项
    Types::ListMediaParams list_params;
    list_params.filter_type = "photo";
    list_params.per_page = 1000;  // 批量处理

    auto media_list_result = Database::list_media_items(app_state, list_params);
    if (!media_list_result) {
      return std::unexpected("Failed to get media list: " + media_list_result.error());
    }

    auto media_list = media_list_result.value();
    std::vector<Types::MediaItem> items_needing_thumbnails;

    // 检查哪些项目需要生成缩略图
    for (const auto& item : media_list.items) {
      auto needs_regen_result = Thumbnail::needs_thumbnail_regeneration(item);
    ...
    // 批量生成缩略图
    auto thumbnails_result = Thumbnail::batch_generate_thumbnails(
...
auto get_media_stats(Core::State::AppState& app_state, const Types::GetStatsParams& params)
  ...
  return Database::get_media_stats(app_state, params);
}
...
auto cleanup_deleted_items(Core::State::AppState& app_state, int days_old)
    -> std::expected<Types::MediaOperationResult, std::string> {
  try {
    auto cleanup_result = Database::cleanup_soft_deleted(app_state, days_old);
...
auto verify_media_integrity(Core::State::AppState& app_state)
    -> std::expected<Types::ScanResult, std::string> {
  try {
    Logger().info("Starting media integrity verification...");
    ...
    Types::ScanResult result;
    result.total_files = media_list.total_count;
    result.new_items = 0;
    result.updated_items = 0;
    result.deleted_items = 0;
    ...
    for (const auto& item : media_list.items) {
      ...
      if (item.type == "photo" && item.thumbnail_path.has_value()) {
        auto thumbnail_path_result = Thumbnail::get_thumbnail_path(item);
        if (thumbnail_path_result) {
          auto thumbnail_path = thumbnail_path_result.value();
          if (!Thumbnail::thumbnail_exists(thumbnail_path)) {
    ...
    Logger().info("Media integrity verification completed. Errors found: {}", result.errors.size());

    return result;

  } catch (const std::exception& e) {
    return std::unexpected("Exception in verify_media_integrity: " + std::string(e.what()));
  }
}

auto rebuild_thumbnail_index(Core::State::AppState& app_state)
    -> std::expected<Types::ScanResult, std::string> {
  try {
    ...
    // 重新生成所有缩略图
    auto generate_result = generate_missing_thumbnails(app_state, 400, 400);
    if (!generate_result) {
...
auto get_default_scan_options() -> Types::ScanOptions {
...
auto validate_scan_params(const Types::ScanMediaParams& params)
