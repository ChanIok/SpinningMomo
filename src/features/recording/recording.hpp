...
auto enter_stopping(Core::State::AppState& app_state) -> bool;

// 等待录制控制线程退出，用于应用关闭阶段完成录制收尾。
auto join_control_thread(Core::State::AppState& app_state) -> void;

// 发送一条录制相关的纯文字通知，例如“录制已开始”或启动前校验失败。
auto notify_message(Core::State::AppState& app_state, const std::string& message) -> void;

// 当录制正在停止并封装输出文件时，提示用户当前仍在收尾阶段。
auto notify_stopping(Core::State::AppState& app_state) -> void;
