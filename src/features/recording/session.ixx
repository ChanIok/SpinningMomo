...
export auto clear_session_runtime_fields(Features::Recording::State::RecordingState& state) -> void;

// 取消延迟释放 D3D 资源的定时器。
export auto cancel_cleanup_timer(Features::Recording::State::RecordingState& state) -> void;

// 启动延迟释放 D3D 资源的定时器，高频启停时避免反复初始化设备。
export auto start_cleanup_timer(Features::Recording::State::RecordingState& state,
                                std::function<void()> on_timeout) -> void;

// 立即释放录制模块可复用的 D3D / WinRT 设备资源。
export auto cleanup_d3d_resources(Features::Recording::State::RecordingState& state) -> void;

// 确保录制所需的 D3D device/context 和 WinRT device 已创建。
export auto ensure_d3d_resources_ready(Features::Recording::State::RecordingState& state)
    -> std::expected<void, std::string>;

// 停止 WGC 继续产帧，但保留 frame pool 供编码线程排空。
export auto stop_capture_input(Features::Recording::State::RecordingState& state) -> void;

// 清理当前 WGC 捕获会话资源。
export auto cleanup_capture_session(Features::Recording::State::RecordingState& state) -> void;
