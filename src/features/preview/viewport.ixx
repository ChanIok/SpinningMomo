module;

export module Features.Preview.Viewport;

import std;
import Core.State;
import <d3d11.h>;
import <wil/com.h>;

namespace Features::Preview::Viewport {

// 更新视口矩形状态
export auto update_viewport_rect(Core::State::AppState& state) -> void;

// 渲染视口框到屏幕
export auto render_viewport_frame(Core::State::AppState& state, ID3D11DeviceContext* context,
                                  const wil::com_ptr<ID3D11VertexShader>& vertex_shader,
                                  const wil::com_ptr<ID3D11PixelShader>& pixel_shader,
                                  const wil::com_ptr<ID3D11InputLayout>& input_layout) -> void;

}  // namespace Features::Preview::Viewport