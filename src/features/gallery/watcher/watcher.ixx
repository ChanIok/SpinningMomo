...
// 标记应用主动操作的源/目标路径进入 watcher 忽略集合。
export auto begin_manual_file_system_ignore(Core::State::AppState& app_state,
                                            const std::filesystem::path& source_path,
                                            const std::filesystem::path& destination_path)
    -> std::expected<void, std::string>;

// 应用主动操作完成后结束 in-flight，并保留短缓冲吸收延迟通知。
export auto complete_manual_file_system_ignore(Core::State::AppState& app_state,
                                               const std::filesystem::path& source_path,
                                               const std::filesystem::path& destination_path)
    -> std::expected<void, std::string>;
