...
// 内部辅助函数声明
auto get_app_state_from_hwnd(HWND hwnd) -> Core::State::AppState*;
...
auto start_event_thread(Core::State::AppState& state, const State::LetterboxConfig& config)
    -> std::expected<void, std::string>;
...
auto show(Core::State::AppState& state, HWND target_window) -> std::expected<void, std::string> {
  ...
  // 确保事件监听线程已启动
  if (!is_event_thread_running(state)) {
    if (auto result = start_event_monitoring(state); !result) {
...
auto get_app_state_from_hwnd(HWND hwnd) -> Core::State::AppState* {
  return reinterpret_cast<Core::State::AppState*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
}
...
auto create_letterbox_window(State::LetterboxState& letterbox, Core::State::AppState* state)
  ...
  // 关联窗口与状态
  SetWindowLongPtr(letterbox.window_handle, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(state));
...
LRESULT CALLBACK letterbox_wnd_proc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
  auto* state = get_app_state_from_hwnd(hwnd);
  if (!state) {
