...
// INI 文件读写辅助函数
auto read_string(const std::wstring& path, const std::wstring& section, const std::wstring& key,
                 const std::wstring& default_val) -> std::wstring {
  wchar_t buffer[1024];
  GetPrivateProfileStringW(section.c_str(), key.c_str(), default_val.c_str(), buffer,
                           _countof(buffer), path.c_str());
  return buffer;
}

auto write_string(const std::wstring& path, const std::wstring& section, const std::wstring& key,
                  const std::wstring& value) -> bool {
  return WritePrivateProfileStringW(section.c_str(), key.c_str(), value.c_str(), path.c_str()) != 0;
}

auto read_int(const std::wstring& path, const std::wstring& section, const std::wstring& key,
              int default_val) -> int {
  return GetPrivateProfileIntW(section.c_str(), key.c_str(), default_val, path.c_str());
}

auto write_int(const std::wstring& path, const std::wstring& section, const std::wstring& key,
               int value) -> bool {
  return write_string(path, section, key, std::to_wstring(value));
}

// 列表解析与序列化
auto parse_string_list(const std::wstring& str, wchar_t delimiter = L',')
...
auto serialize_string_list(const std::vector<std::wstring>& list, wchar_t delimiter = L',')
    -> std::wstring {
...
auto create_default_config_file(const std::wstring& path) -> std::expected<void, std::string> {
  ...
  bool success =
      ...
      write_string(path, Constants::MENU_SECTION, Constants::MENU_ITEMS,
                   L"CaptureWindow,OpenScreenshot,PreviewWindow,OverlayWindow,LetterboxWindow,"
                   L"Reset,Close,Exit") &&
      write_string(path, Constants::MENU_SECTION, Constants::ASPECT_RATIO_ITEMS,
                   L"32:9,21:9,16:9,3:2,1:1,3:4,2:3,9:16") &&
      write_string(path, Constants::MENU_SECTION, Constants::RESOLUTION_ITEMS,
...
// 默认预设获取
auto get_default_ratio_presets() -> std::vector<Types::Presets::RatioPreset> {
  return {{L"32:9", 32.0 / 9.0}, {L"21:9", 21.0 / 9.0}, {L"16:9", 16.0 / 9.0},
          {L"3:2", 3.0 / 2.0},   {L"1:1", 1.0},         {L"3:4", 3.0 / 4.0},
          {L"2:3", 2.0 / 3.0},   {L"9:16", 9.0 / 16.0}};
}

auto get_default_resolution_presets() -> std::vector<Types::Presets::ResolutionPreset> {
  return {{L"Default", 0, 0},  {L"1080P", 1920, 1080}, {L"2K", 2560, 1440},  {L"4K", 3840, 2160},
          {L"6K", 5760, 3240}, {L"8K", 7680, 4320},    {L"12K", 11520, 6480}};
}

// 自定义配置解析
auto add_custom_ratio(const std::wstring& ratio, std::vector<Types::Presets::RatioPreset>& ratios)
    -> bool {
...
auto add_custom_resolution(const std::wstring& resolution,
                           std::vector<Types::Presets::ResolutionPreset>& resolutions) -> bool {
  try {
...
auto save(const Core::Config::State::AppConfig& config) -> std::expected<void, std::string> {
  ...
  bool success =
      ...
      write_string(path, Constants::LANG_SECTION, Constants::LANG_CURRENT,
                   config.language.current_language) &&
      write_int(path, Constants::TASKBAR_SECTION, Constants::TASKBAR_AUTOHIDE,
                config.taskbar.auto_hide) &&
      write_int(path, Constants::TASKBAR_SECTION, Constants::TASKBAR_LOWER, config.taskbar.lower) &&
      write_int(path, Constants::MENU_SECTION, Constants::MENU_FLOATING,
                config.menu.use_floating_window) &&
      write_string(path, Constants::MENU_SECTION, Constants::MENU_ITEMS,
                   serialize_string_list(config.menu.items_to_show)) &&
      write_string(path, Constants::MENU_SECTION, Constants::ASPECT_RATIO_ITEMS,
                   serialize_string_list(config.menu.aspect_ratio_items)) &&
      write_string(path, Constants::MENU_SECTION, Constants::RESOLUTION_ITEMS,
                   serialize_string_list(config.menu.resolution_items)) &&
      write_string(path, Constants::SCREENSHOT_SECTION, Constants::SCREENSHOT_PATH,
                   config.game_album.screenshot_path) &&
      write_int(path, Constants::LETTERBOX_SECTION, Constants::LETTERBOX_ENABLED,
...
auto get_aspect_ratios(const Core::Config::State::AppConfig& config,
  ...
  std::vector<Types::Presets::RatioPreset> configured_ratios;
  std::wstring error_details;

  for (const auto& item_name : config.menu.aspect_ratio_items) {
    bool found = false;
    if (auto preset_it = std::ranges::find_if(default_presets,
                                              [&](const auto& p) { return p.name == item_name; });
        preset_it != default_presets.end()) {
      configured_ratios.push_back(*preset_it);
      found = true;
    }

    if (!found) {
      if (!add_custom_ratio(item_name, configured_ratios)) {
        error_details += item_name + L",";
      }
    }
  }
  ...
  if (!error_details.empty()) {
    error_details.resize(error_details.length() - 1);  // remove trailing comma
    result.success = false;
    result.error_details = strings.CONFIG_FORMAT_ERROR + L" " + error_details;
  }
...
auto get_resolution_presets(const Core::Config::State::AppConfig& config,
  ...
  std::vector<Types::Presets::ResolutionPreset> configured_resolutions;
  std::wstring error_details;

  for (const auto& item_name : config.menu.resolution_items) {
    bool found = false;
    if (auto preset_it = std::ranges::find_if(default_presets,
                                              [&](const auto& p) { return p.name == item_name; });
        preset_it != default_presets.end()) {
      configured_resolutions.push_back(*preset_it);
      found = true;
    }

    if (!found) {
      if (!add_custom_resolution(item_name, configured_resolutions)) {
        error_details += item_name + L",";
      }
    }
  }
