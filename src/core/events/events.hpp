...
// 同步发送事件
template <typename T>
inline auto send(Core::State::AppState& state, const T& event) -> void {
  send_event(state, std::type_index(typeid(T)), std::any(event));
}

// 异步投递事件
template <typename T>
inline auto post(Core::State::AppState& state, T event) -> void {
  post_event(state, std::type_index(typeid(T)), std::any(std::move(event)));
}

// 订阅强类型事件：把处理器移动进总线并在类型擦除边界完成 any_cast
template <typename T>
inline auto subscribe(Core::State::AppState& state,
                      std::move_only_function<void(const T&) const> handler) -> void {
  if (!handler) {
    return;
  }
