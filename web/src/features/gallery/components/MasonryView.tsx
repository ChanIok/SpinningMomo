...
export function MasonryView({}: MasonryViewProps = {}) {
  // 直接从 store 获取数据
  ...
  const fetchMorePhotos = useCallback(async (index: number) => {
    ...
    try {
      ...
      console.log(`Loading batch ${index}, would load items ${index * 50} to ${(index + 1) * 50 - 1}`)
  ...
  return (
    <div className='w-full' style={{ height: containerHeight }}>
      <InfiniteScroll
        ...
        error={<div className="flex justify-center p-4">
          <div className="text-destructive">加载失败，请稍后重试</div>
        </div>}
        finished={<div className="flex justify-center p-4">
