...
export function AdaptiveView() {
  ...
  const targetRowHeight = useMemo(() => {
    const sizeMultiplier = (viewConfig.size - 1) * 0.4 + 1 // 1-2.6倍
    return (containerWidth: number) => {
      // 响应式行高：大屏幕更高，小屏幕较低
      if (containerWidth >= 1200) return Math.floor(240 * sizeMultiplier)
      if (containerWidth >= 800) return Math.floor(200 * sizeMultiplier)
      if (containerWidth >= 600) return Math.floor(160 * sizeMultiplier)
      return Math.floor(120 * sizeMultiplier)
    }
  }, [viewConfig.size])
