...
const createGhost = (source: HTMLElement, rect: DOMRect, initialTop: number): HTMLElement => {
    ...
    ghost.classList.remove('bg-card', 'border', 'border-transparent', 'hover:bg-accent/50', 'hover:border-primary/20')
...
const cacheItemRects = () => {
    ...
    const itemElements = Array.from(itemsContainer.value.querySelectorAll('.draggable-item')) as HTMLElement[]
...
const handlePointerMove = (event: PointerEvent) => {
    ...
    if (!draggingId.value && sourceEl.value && Math.abs(deltaY) > 3) {
        ...
        const itemIndex = Array.from(itemsContainer.value?.querySelectorAll('.draggable-item') || [])
...
<template>
  <div class="space-y-4">
    ...
    <div class="rounded-lg border bg-card p-4 text-card-foreground shadow-sm">
        <div ref="itemsContainer" class="flex flex-col gap-1 relative">
            <TransitionGroup name="list">
                <div
                    v-for="item in items"
                    :key="item.id"
                    class="draggable-item flex items-center justify-between rounded-md p-3 transition-colors hover:bg-accent/50 group border border-transparent hover:border-primary/20 bg-card cursor-grab active:cursor-grabbing"
                    :class="{
                ...
                >
                    <div class="flex-1 pr-4 flex items-center gap-3">
                         <div class="text-muted-foreground/60 group-hover:text-muted-foreground transition-colors">
                            <GripVertical class="h-4 w-4" />
                         </div>
                         <Label class="text-sm font-medium text-foreground pointer-events-none select-none">
                    ...
                    <div class="flex flex-shrink-0 items-center gap-2">
                        <Switch
                            ...
                            @pointerdown.stop
                        />
                         <span class="min-w-[3rem] text-xs text-muted-foreground text-right pointer-events-none">
        ...
        <div v-if="allowAdd" class="mt-4 pt-4 border-t">
