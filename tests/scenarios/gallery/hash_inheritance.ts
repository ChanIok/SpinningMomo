...
await runScenario("gallery/hash_inheritance", async (application, environment) => {
  const sourcePath = await copyScenarioFixture(environment, "source.png");
  ...
  const duplicatePath = await copyScenarioFixture(environment, "duplicate.png");
  await scanDirectory(application, environment.galleryDirectory);
