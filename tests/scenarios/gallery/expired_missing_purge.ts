...
const phase: ScenarioPhase = {
  name: "expired_missing_purge",
  // 启动时应删除超过宽限期的 Missing 资产，并清理不再被任何资产引用的缩略图。
  // 缩略图按内容 hash 共享，因此必须使用内容唯一的 green fixture，
  // 确保清理后没有其他资产仍引用同一缩略图。
  action: async (application, environment) => {
    const assetPath = await copyScenarioFixture(environment, "expired.png", "green");
...
if (isMainModule(import.meta.url)) {
  await runScenario("gallery/expired_missing_purge", phase.action);
}
