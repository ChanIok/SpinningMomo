...
const rules = [
  ...
  {
    id: "uwebsockets",
    packageDirs: ["vcpkg_uwebsockets"],
    version: {
      file: "share/uwebsockets/vcpkg.spdx.json",
      expect: '"versionInfo": "20.77.0"',
    },
    files: [
      {
        file: "include/libusockets.h",
        replacements: [
          {
            before: [
              "struct us_socket_t;",
      ...
      {
        file: "include/uwebsockets/ChunkedEncoding.h",
        replacements: [
          {
            before:
              "    static std::optional<std::string_view> getNextChunk(std::string_view &data, uint64_t &state, bool trailer = false) {",
            after:
              "    inline std::optional<std::string_view> getNextChunk(std::string_view &data, uint64_t &state, bool trailer = false) {",
          },
        ],
      },
      {
        file: "include/uwebsockets/HttpParser.h",
        replacements: [
          {
            before: "static const unsigned int MINIMUM_HTTP_POST_PADDING = 32;",
            after: "inline constexpr unsigned int MINIMUM_HTTP_POST_PADDING = 32;",
          },
        ],
      },
      {
        file: "include/uwebsockets/LoopData.h",
        replacements: [
          {
            before: [
              "    void updateDate() {",
      ...
      {
        file: "include/uwebsockets/Loop.h",
        replacements: [
          {
            before: [
            ...
            after: [
              "    /* What to do with loops created with existingNativeLoop? */",
              "    struct LazyLoopState {",
              "        Loop *loop = nullptr;",
              "        bool cleanMe = false;",
              "    };",
              "",
              "    static LazyLoopState &getLazyLoop() {",
              "        static thread_local LazyLoopState lazyLoop;",
              "        return lazyLoop;",
              "    }",
            ].join("\n"),
          },
      ...
      {
        file: "include/uwebsockets/CachingApp.h",
        replacements: [
          {
            before: "        created = time(0);",
            after: "        created = _time64(nullptr);",
          },
        ],
      },
    ],
